...
class Gui:
    ...
    def __init__(self):
        ...
        # Setup frames
        self.debug_frame = DebugFrame(self.root_widget)
        self.error_frame = ErrorFrame(self.root_widget)
        self.visualisation_frame = VisualisationFrame(self.root_widget)
        self.graph_frame = GraphFrame(self.root_widget)
    ...
    # Refresh the gui
    def update(self):
        self.graph_frame.graph_refresh(self.accel_data)
    ...
    def graph_data(self, element, data):
        self.accel_data[element].append(data)
...
class GraphFrame:
    ...
    def __init__(self, parent_widget):
        ...
        self.graph.set_xlim(self.x_range)
    ...
    # Update the graph. We have to re-plot the whole graph every time. If the plot nears the edge, shift the range.
    def graph_refresh(self, accel_data):
        while accel_data["ACCEL_TIME"][-1] >= self.x_range[1]:
            self.x_range[0] += 10000
            self.x_range[1] += 10000
        else:
            self.graph.set_xlim(self.x_range)

        # Find the limit of the complete sets of data
        limit = min(len(accel_data["ACCEL_TIME"]), len(accel_data["ACCEL_X"]),
                    len(accel_data["ACCEL_Y"]), len(accel_data["ACCEL_Z"]))

        self.graph.plot(accel_data["ACCEL_TIME"][:limit],
                        accel_data["ACCEL_X"][:limit], color="red")
        self.graph.plot(accel_data["ACCEL_TIME"][:limit],
                        accel_data["ACCEL_Y"][:limit], color="green")
        self.graph.plot(accel_data["ACCEL_TIME"][:limit],
                        accel_data["ACCEL_Z"][:limit], color="blue")

        self.accel_data = [[accel_data["ACCEL_TIME"][-1]], [accel_data["ACCEL_X"][-1]],
                           [accel_data["ACCEL_Y"][-1]], [accel_data["ACCEL_Z"][-1]]]
        self.canvas_graph.draw()
